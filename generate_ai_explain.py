...
def build_prompt(q):
    ...
    p += '【术语标注】当中文里出现对应日文片假名外来语（如イテレーション、スタック、キャッシュ等）时，在该中文词后面标注 片假名：英文原文，例如 迭代（イテレーション：iteration）。\n'
    p += '【缩写标注】当中文里出现IT缩写（如UDDI、SOA、API、DNS等）时，首次出现标注 英文全称：中文全称，例如 UDDI（Universal Description, Discovery and Integration：统一描述、发现和集成）。只标注重要缩写，不要过度标注。\n\n'
